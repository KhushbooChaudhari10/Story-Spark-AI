...
interface DrawingPageProps {
  onComplete?: () => void; // ✅ optional callback to trigger next page
}

export default function DrawingPage({ onComplete }: DrawingPageProps) {
  ...
  // ✅ Ensure hydration-safe mounting
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  useEffect(() => {
    if (colorPickerRef.current) {
      colorPickerRef.current.value = selectedColor;
    }
  }, [selectedColor]);

  // ✅ Initialize canvas after refs + mount
  useEffect(() => {
    if (!mounted) return;
    ...
    // ✅ Optional: draw background image once
    // const background = new Image();
    // background.src = "/c0d5f41f-cf89-4a0f-9fc3-066b2de8b3a1.jpg"; // put in public/ folder
    // background.onload = () => {
  ...
  return (
    <main className="flex flex-col items-center min-h-screen bg-cover bg-center p-6"
    style={{
      backgroundImage: "url('/c0d5f41f-cf89-4a0f-9fc3-066b2de8b3a1.jpg')",
    }}
  >
      ...
      <div className="flex flex-wrap items-center justify-center gap-4 mb-4 bg-purple-400 p-4 shadow-md rounded-lg w-full max-w-5xl">
        ...
        <div className="flex gap-2">
          {presetColors.map((color) => (
            <button
              ...
              className={`w-8 h-8 rounded-full border ${selectedColor === color ? "outline-2 outline-black" : ""
                }`}
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg text-white ${erasing ? "bg-red-700" : "bg-purple-700"
            }`}
        >
      ...
      <div className="flex gap-2 mb-4">
        {[
          ...
          <button
            ...
            className={`px-3 py-2 rounded ${shapeMode === btn.shape
